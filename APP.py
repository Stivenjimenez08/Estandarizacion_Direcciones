...
def cargar_scripts_normalizacion():
    """
    Busca los scripts definidos en SCRIPTS dentro de la misma carpeta que app.py.
    Es tolerante a mayúsculas/minúsculas en los nombres de archivo.
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))

    # listado real de archivos .py en la carpeta
    archivos_py = [f for f in os.listdir(base_dir) if f.lower().endswith(".py")]
    # mapa en minúsculas -> nombre real
    mapa_archivos = {f.lower(): f for f in archivos_py}

    modulos = []

    for nombre, archivo_deseado in SCRIPTS:
        clave = archivo_deseado.lower()
        archivo_real = mapa_archivos.get(clave)

        if archivo_real is None:
            st.warning(
                f"Script esperado `{archivo_deseado}` no se encontró en la carpeta del proyecto. "
                f"Archivos disponibles: {', '.join(archivos_py)}"
            )
            continue

        ruta_script = os.path.join(base_dir, archivo_real)

        try:
            modulo = cargar_modulo_desde_archivo(ruta_script, nombre)
        except Exception as e:
            st.warning(f"Error cargando `{archivo_real}`: {e}")
            continue

        if not hasattr(modulo, "procesar"):
            st.warning(
                f"El script `{archivo_real}` no define la función `procesar(df)`. "
                f"Se omitirá."
            )
            continue

        modulos.append((nombre, modulo))
        st.success(f"✅ Script cargado: `{archivo_real}` (alias: {nombre})")
